...
export default function Dashboard() {
  ...
  if (error) {
    return (
      <div className="p-4 sm:p-6 text-center">
        ...
        <Button onClick={fetchDashboardData} size="sm" className="text-xs sm:text-sm">{t('try_again')}</Button>
      </div>
    )
  }
  ...
  if (!dashboardData) {
    return (
      <div className="p-4 sm:p-6 text-gray-600 text-center">
      ...
      <div className="p-3 sm:p-4 md:p-6 space-y-4 md:space-y-6">
        ...
        <div className="flex flex-col gap-3 sm:gap-4 md:flex-row md:justify-between md:items-start">
          <div className="space-y-1">
            <h1 className="text-xl sm:text-2xl md:text-3xl font-bold text-gray-900">{t('welcome')}, {user.fullName}</h1>
            ...
            <CardContent className="p-3 sm:p-4">
              <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
                <div className="flex items-start gap-3">
                  ...
                  <div>
                    <h3 className="font-medium text-yellow-900 text-sm sm:text-base">{t('complete_profile')}</h3>
                ...
                <Button
                  ...
                  className="bg-yellow-600 hover:bg-yellow-700 w-full sm:w-auto text-xs sm:text-sm"
                >
                  {t('complete_profile')}
                </Button>
              </div>
        ...
        <div className="grid grid-cols-2 sm:grid-cols-2 md:grid-cols-4 gap-3 sm:gap-4">
          <Card>
            <CardContent className="p-3 sm:p-4">
              <div className="flex items-center justify-between">
                <div>
                  <p className="text-xs sm:text-sm font-medium text-gray-600">{t('profile')}</p>
                  <p className="text-lg sm:text-xl md:text-2xl font-bold text-gray-900">{profileCompletion}%</p>
                </div>
                <div className="h-6 w-6 sm:h-8 sm:w-8 bg-teal-100 rounded-full flex items-center justify-center">
          ...
          <Card>
            <CardContent className="p-3 sm:p-4">
              <div className="flex items-center justify-between">
                <div>
                  <p className="text-xs sm:text-sm font-medium text-gray-600">{t('progress')}</p>
                  <p className="text-lg sm:text-xl md:text-2xl font-bold text-gray-900">{overallProgress}%</p>
                </div>
          ...
          <Card>
            <CardContent className="p-3 sm:p-4">
              <div className="flex items-center justify-between">
                <div>
                  <p className="text-xs sm:text-sm font-medium text-gray-600">{t('notifications')}</p>
                  <p className="text-lg sm:text-xl md:text-2xl font-bold text-gray-900">{notifications.filter((n) => !n.read).length}</p>
                </div>
        ...
        {!hasStartedRegistration ? (
          <Card className="border-2 border-dashed border-gray-300 bg-gray-50">
            <CardContent className="flex flex-col items-center justify-center py-8 sm:py-12">
              ...
              <h3 className="text-lg sm:text-xl font-semibold text-gray-900 mb-2 text-center">{t('get_started_title')}</h3>
            ...
            <Card>
              <CardHeader className="pb-3 sm:pb-4">
                <div className="flex flex-col sm:flex-row sm:justify-between sm:items-start gap-3">
                  <div>
                    <CardTitle className="text-base sm:text-lg md:text-xl">{t('overall_registration_progress')}</CardTitle>
                    <CardDescription className="text-sm sm:text-base">{t('export_ready_journey')}</CardDescription>
                  </div>
                  <Link href="/dashboard/progress">
                    <Button
                      variant="outline"
                      size="sm"
                      className="bg-teal-50 text-teal-700 border-teal-200 hover:bg-teal-100 w-full sm:w-auto text-xs sm:text-sm"
                    >
              ...
              <CardContent>
                <div className="space-y-3 sm:space-y-4">
                  <div className="flex items-center justify-between">
                    <span className="text-xs sm:text-sm font-medium">{t('overall_progress')}</span>
                    <span className="text-xs sm:text-sm text-gray-600">{overallProgress}%</span>
                  </div>
              ...
              <CardContent className="px-2 sm:px-3 md:px-6">
                            ...
                            {index !== currentVideoIndex && (
                              <div className="absolute inset-0 bg-black bg-opacity-40 flex items-center justify-center">
                                <div className="w-8 h-8 sm:w-10 sm:h-10 md:w-12 md:h-12 bg-white bg-opacity-90 rounded-full flex items-center justify-center">
                                  <div className="w-0 h-0 border-l-2 sm:border-l-3 md:border-l-4 border-l-gray-700 border-t-1 sm:border-t-2 border-t-transparent border-b-1 sm:border-b-2 border-b-transparent ml-0.5 sm:ml-1"></div>
                                </div>
                              </div>
                            )}
                          </div>
                          <div className="p-2 sm:p-3 md:p-4">
                            <h3 className="font-medium text-gray-900 mb-1 text-xs sm:text-sm md:text-base">{video.title}</h3>
                            <p className="text-xs sm:text-sm text-gray-600 mb-2">{video.description}</p>
                ...
                <div className="mt-4 sm:mt-6 text-center">
                  <Link href="/dashboard/tutorials">
                    <Button
                      variant="outline"
                      className="text-gray-700 border-gray-300 hover:bg-gray-100 bg-transparent text-xs sm:text-sm"
                    >
